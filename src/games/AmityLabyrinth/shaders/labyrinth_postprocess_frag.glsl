...
void main()
{
    ...
    // ==========================================
    // LABYRINTH HORROR FLASHLIGHT SHADER
    // ==========================================

    // 1. Core Beam Mask (2D Screen Circle)
    vec2 screenCenter = vec2(0.5, 0.5);
    float distToCenter = length(TexCoords - screenCenter);

    // Spotlight Cone: inner beam is fully bright, outer beam feathers out
    float innerCutoff = 0.14; // Tight, atmospheric horror flashlight beam
    float outerCutoff = 0.28; 
    float flashlightCone = smoothstep(outerCutoff, innerCutoff, distToCenter);

    // 2. Distance-Based Falloff (Linearize Depth)
    float near = 0.1;
    float far = 5000.0;
    float z_ndc = depth * 2.0 - 1.0; 
    float linearDepth = (2.0 * near * far) / (far + near - z_ndc * (far - near));

    // Calculate light reach (fades to black at 40 units away for a claustrophobic horror feel)
    float maxDistance = 40.0; 
    float distanceAttenuation = clamp(1.0 - (linearDepth / maxDistance), 0.0, 1.0);
    distanceAttenuation = pow(distanceAttenuation, 2.0); // Quadratic falloff

    // If a pixel represents the sky/clouds (infinite depth), the flashlight shouldn't reach it
    if (depth >= 0.99999) {
        distanceAttenuation = 0.0;
    }

    // 3. Apply the Flashlight Illumination Boost
    float ambientLevel = 0.02; // Near total pitch-black darkness for pure horror suspense!
    float flashlightStrength = 2.0; // Strong central beam
    
    float totalIllumination = ambientLevel + (flashlightCone * distanceAttenuation * flashlightStrength);
    
    // Apply this illumination multiplier to our base scene color
    color = color * totalIllumination;
    // ==========================================
